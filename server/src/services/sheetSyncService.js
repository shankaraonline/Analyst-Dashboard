...
export function cleanValue(val) {
  ...
  // Normal pure number with commas (e.g. "1,250,000" or "-50")
...
/**
 * Infers the data type and format for a column based on its values and header name
 */
...
export function inferColumnType(header = '', sampleValues = []) {
  ...
  // Inspect sample values first for unambiguous date/time patterns
  ...
  if (nonEmpty.length > 0) {
    ...
    const emailCount = nonEmpty.filter(v => String(v).includes('@') && String(v).includes('.')).length;
    if (emailCount / nonEmpty.length > 0.4) return 'text';
  }
...
/**
 * Creates a URL-safe, clean tab ID / slug
 */
...
/**
 * Universal dynamic parser for any worksheet
 */
export function parseWorksheetData(worksheet, sheetName = 'Sheet1') {
  ...
  // 4. Refine column types based on actual row values
...
/**
 * Fetches and parses any multi-tab Google Sheet completely dynamically
 */
export async function fetchAndParseGoogleSheet(sheetUrl) {
  const sheetId = extractSpreadsheetId(sheetUrl);
  if (!sheetId) {
    throw new Error('Invalid Google Sheet URL. Please ensure it is a valid Google Spreadsheet link.');
  }

  const exportUrl = `https://docs.google.com/spreadsheets/d/${sheetId}/export?format=xlsx`;
  const response = await fetch(exportUrl);
  if (!response.ok) {
    throw new Error(`Google Sheet export returned status ${response.status}. Please verify that the sheet sharing is set to "Anyone with the link can view".`);
  }
  ...
  if (sheetNames.length === 0) {
    throw new Error('No sheets/tabs found in this Google Spreadsheet.');
  }
  ...
  sheetNames.forEach(sName => {
    ...
    try {
      ...
      console.warn(`Could not parse tab "${sName}":`, tabErr.message);
    }
  });
